...
cmd({
    ...
    if (!args[0]) {
        return reply(`📌 Current mode: *${config.MODE}*\n\n_Usage: .mode private .mode public .mode group .mode inbox_`);
    }
    ...
    } else if (modeArg === "group") {
        config.MODE = "group";
        return reply("✅ _BOT MODE IS NOW SET TO_ *GROUP*.");
    } else if (modeArg === "inbox") {
        config.MODE = "inbox";
        return reply("✅ _BOT MODE IS NOW SET TO_ *INBOX*.");
    }
    else {
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    const status = args[0]?.toLowerCase();
    if (!["on", "off"].includes(status)) {
        return reply("*🫟 ᴇxᴀᴍᴘʟᴇ: .fakerecording*");
    }
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply(`*🫟 ᴇxᴀᴍᴘʟᴇ:  .autoreadstatus on*`);
    }
}); 
//--------------------------------------------
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply("anticall of statuses is now enabled.");
    } else if (args[0] === "off") {
        config.ANTICALL = "false";
        return reply("anticall of statuses is now disabled.");
    } else {
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply(`_example:  .statusreply on_`);
    }
});
//-------------------------------------------
//INBOX-BLOCK
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply("*inbox block is now enabled.*");
    } else if (args[0] === "off") {
        config.INBOX_BLOCK = "false";
        return reply("*inbox block is now disabled*");
    } else {
        return reply(`_example:  .inboxblock on_`);
    }
});
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply("*anti delete is now enabled.*");
    } else if (args[0] === "off") {
        config.ANTI_DELETE = "false";
        return reply("*anti delete is now disabled*");
    } else {
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        return reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .autoreply on*`);
    }
});
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .autoreact on*`);
    }
});
//---------------CUSTOME REACT
cmd({
    pattern: "customereact",
    alias: ["custreact"],
    desc: "Enable or disable the autoreact feature",
    category: "settings",
    filename: __filename
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        await reply("*_CUSTOM_REACT IS NOW ENABLED_*");
    } else if (args[0] === "off") {
        config.CUSTOM_REACT = "false";
        await reply("*_CUSTOM_REACT IS NOW DISABLED_*");
    } else {
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .customereact on*`);
    }
});
...
cmd({
    ...
    if (["off", "warn", "delete", "kick"].includes(action)) {
        ...
        return reply("*🫟 ᴇxᴀᴍᴘʟᴇ: .antibot off/warn/delete/kick*");
    }
});
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        config.WELCOME = "true";
        await reply("*_WELCOME MSG IS NOW ENABLED_.*");
    } else if (args[0] === "off") {
        config.WELCOME = "false";
        await reply("*_WELCOME MSG IS NOW DISABLED_.*");
    } else {
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .welcome on*`);
    }
});
//------------------------------------------
//      GOODBYE
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        config.GOODBYE = "true";
        await reply("*_GOOD BYE MSG IS NOW ENABLED_.*");
    } else if (args[0] === "off") {
        config.GOODBYE = "false";
        await reply("*_GOODBYE MSG IS NOW DISABLED_.*");
    } else {
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .goodbye on*`);
    }
});
//----------ADMIN_EVENTS
cmd({
...
async (conn, mek, m, { from, args, isOwner, reply }) => {
    ...
    if (args[0] === "on") {
        ...
        await reply("*ADMIN_EVENTS IS NOW ENABLED_.*");
    } else if (args[0] === "off") {
        config.ADMIN_EVENTS = "false";
        await reply("*ADMIN_EVENTS IS NOW DISABLED_.*");
    } else {
        await reply(`*🫟 ᴇxᴀᴍᴘʟᴇ: .adminevents on*`);
    }
});
//--------------------------------------------
//   POLL COMMANDS
